...
- **今晚打老虎**：一輪最多 5 題，間隔由 FSRS 依每張卡的實際表現決定。答完一題就停下來，隨時可以「今天先到這裡」。
...
![用學習足跡查看收藏紀錄](docs/images/learning-activity.png)
...
| 設定頁「我的攔路虎」 | 收合列總表：搜尋、依學習進度篩選、依下次複習排序 |
| 設定頁「學習足跡」 | 用月曆查看每天的收藏、來源文章與打老虎成績 |
...
- 「今晚打老虎」只挑選已到期的卡片與尚未打過的新卡，兩者都需要 AI 詞典（片語另外需要來源語境）才出題。
- 間隔由 `ts-fsrs` 決定，關掉分鐘級 learning steps，下次複習日對齊當地午夜。同一張卡不會在同一晚再出現。
- 「我的攔路虎」的五個進度分類：現在可練、尚未打過、排程中、已馴服、全部。按 `X` 排除的字標成「已排除」，不列入學習進度。
- 「學習足跡」只根據收藏、語境與複習成績產生，不記錄一般瀏覽歷史。
...
- `entrypoints/options/`：設定、收藏、複習與學習足跡。
