...
- 閱讀卡片只在標題列放一個 28px、`opacity: .92` 的 icon。未互動時保持靜止；游標移入與
  ...
  不讓輔助技術重複朗讀。
- 2026-08-26 修訂：所有一般網頁預設顯示 50px 的小虎懸浮球，作為陪伴型啟動器。
  點擊與 `Alt+U` 共用同一個高亮切換流程；以光環表示開啟狀態，只在進場、hover、按下與
  切換時做短促動作，不持續循環。懸浮球可在視窗內拖曳避讓內容，10px 內仍視為點擊，
  避免輕微手震誤判。圖片使用 128px 品牌資產縮放顯示，兼顧高 DPI 螢幕。這是「吉祥物只在卡片」
  限制的明確例外。
...
Edge 與 Safari 封包共用同一套名稱、icon 與 UI，而閱讀卡片仍以文字為主。懸浮球預設在一般
網頁顯示；動畫只回應直接互動、載入、完成或狀態變更。卡片方向固定，長回答在正文
內捲動，使用者也能直接拖開。
IndexedDB 改名後第一次開啟會建立空的 `wordtiger` 資料庫，舊測試資料不會出現；
這是測試階段接受的一次性重置，正式發佈後不可用同一方式任意改名。
`icons/32.png` 與 `icons/128.png` 必須維持在 `web_accessible_resources`，否則網頁中的 Shadow DOM
卡片或懸浮球無法載入它。
