...
一輪五張不同卡片，答完整輪算抓到一隻老虎。五張是上限，不是每日目標；中途離開不需要按鈕，成績一題一題寫進資料庫。
...
既有 `reviewLog` 保留，繼續作為「練過 N 次／抓到 C 次」與老虎足跡的唯一歷史來源；它不參與 FSRS 初始化，也不增加欄位。
...
runtime message 不再只回傳 boolean；成功時一併回傳 `nextReviewAt`。畫面不顯示這個日期，只用它區分「寫入成功」與「要重試」。
...
題的單位是「題」，輪的單位是「隻」：答完整輪算抓到一隻。卡片可顯示「過去練過 N 次」，但不在回答前顯示成功率，避免過去表現影響當下自評。

沒有中途離開的按鈕。成績一題一題寫進資料庫，關掉分頁或切走都不會掉，也不會被記成失敗。

### 整輪完成後

只有整輪答完（或沒有更多題目）才出現收工畫面，不使用全對慶祝與失敗責備：
...
```text
        ...
        這一輪 4／5 抓到
    今天已練 12 題。溜走的會再排回來，不用追。

          [ 再抓一輪 ]
```

畫面不顯示下次複習日期。`reviewWord` 仍回傳 `nextReviewAt`，但只用來判斷是否寫入成功：`null` 代表沒存進去，顯示可重試錯誤並留在原題。
...
篩選列下方固定顯示一行說明：今晚打老虎從「今天排到」和「還沒練過」兩堆各抓，湊滿 5 題。
沒有這一行，「今天排到」會被讀成「所有現在能練的字」，但新卡其實也會被選進來。
...
```text
deploy
遇到 3 次 · 練過 3 次 · 抓到 2 次 · 排程中

rollback
遇到 1 次 · 還沒練過，去今晚打老虎遇遇看
```

「遇到」數的是保留下來的語境筆數。`addContext` 以 (句子, 網址) 去重，所以同一句在同一頁按幾次 `A` 都只算一次：這個數字講的是在幾個不同語境見過它，不是按鍵次數。語境為零時整段省略。

點擊後才展開 AI 詞典與來源語境。既有搜尋保留，進度篩選就是上面那五個，排序維持「最近加入」與「字母排序」。

單字與片語是另一個獨立的軸：搜尋欄旁邊一個下拉（全部／單字／片語），片語那列另外掛一顆「片語」徽章。兩個軸可以同時成立，進度篩選的數字也跟著單字／片語一起收斂，按下去看到幾筆就先寫幾筆。狀態文字依下列順序取第一個成立的規則，規則會重疊，所以順序是規格的一部分：

1. `status === 'known' && collectedAt === null`：已排除，不列入學習進度。
2. `status === 'known'`：已馴服。
3. 缺少 AI 詞典，片語另缺來源語境：還不能出題，提示先查一次詞。
4. 無 `fsrsCard`：還沒練過。這一格涵蓋兩種處境，文案必須分開：`reviewCount === 0` 是真的沒練過；`reviewCount > 0` 是 1.8.0 之前用舊階梯練過、沒有被重播成 FSRS 狀態的字，寫「升級後排程重新開始」。共用同一句會出現「練過 1 次 · 還沒練過」這種自相矛盾的列。
5. `fsrsCard.due <= now`：今天排到。
6. `scheduled_days >= 30`：漸漸穩定。
7. 其餘：排程中。

第 3 條不能省。選題本來就要求可核對的答案材料，少了這條，沒有詞典的字會永遠掛在「今天排到」卻永遠不出題，總表的數字跟實際題數對不起來。它只出現在「全部」清單，不另開篩選。

總表不顯示紅色 overdue，不顯示下次複習日期，也不把本週與上週互相比較。「下次複習」排序保留，它排的是接下來會遇到的順序，不需要把日期印出來。現有老虎足跡繼續呈現每日收藏與練習事件，不增加 streak。
...
- 總表正確推導還沒練過、遇到次數、練習次數與抓到次數。
- 沒有 AI 詞典的字在總表不算「今天排到」，總表數字與實際題數一致。
- 收藏後已馴服與只按 `X` 排除的單字不會混為一談。
- `reviewWord` 回傳 `null` 時停在原題並顯示可重試錯誤，不推進到下一題。
- 清掉快取後重查會重新呼叫 AI，並把墓碑列救回來。
