...
- [x] **Step 5: 接上 background entrypoint**
...
- [x] **Step 5: 寫 content script entrypoint**
...
const HIGHLIGHT_NAME = 'wordtiger-unknown';
...
export default defineContentScript({
  ...
  async main() {
    // 重複按 Alt+U 時關閉,不重複注入
    if ((window as any).__wordTigerActive) {
      CSS.highlights.delete(HIGHLIGHT_NAME);
      document.getElementById(STYLE_ID)?.remove();
      (window as any).__wordTigerActive = false;
      return;
    }
    (window as any).__wordTigerActive = true;
...
- [x] **Step 6: 把 freq.json 列進 web_accessible_resources**
...
- [x] **Step 7: build 並確認沒有寫進 manifest 的 content_scripts**
...
- [x] **Step 1: 為新的 message type 寫失敗的測試**
...
<!doctype html>
<html>
  <head><meta charset="utf-8" /><title>攔詞虎設定</title></head>
